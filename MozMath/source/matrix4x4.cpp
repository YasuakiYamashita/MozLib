...
namespace moz
{
	namespace math
	{
		...
		// Yé≤âÒì]
		//------------------------------------------------------------------------------
		void Matrix4x4::CreateYawMatrix(Matrix4x4* mat, float rot)
		{
			float cos = cosf(rot), sin = sinf(rot);
			mat->a._11 = cos;
			mat->a._12 = 0;
			mat->a._13 = -sin;
			mat->a._14 = 0;
		...
		// Xé≤âÒì]
		//------------------------------------------------------------------------------
		void Matrix4x4::CreatePitchMatrix(Matrix4x4* mat, float rot)
		{
			float cos = cosf(rot), sin = sinf(rot);
			mat->a._11 = 1;
			mat->a._12 = 0;
		...
		// Zé≤âÒì]
		//------------------------------------------------------------------------------
		void Matrix4x4::CreateRollMatrix(Matrix4x4* mat, float rot)
		{
			float cos = cosf(rot), sin = sinf(rot);
			mat->a._11 = cos;
			mat->a._12 = sin;
		...
		// ëSÇƒÇÃé≤âÒì]
		//------------------------------------------------------------------------------
		void Matrix4x4::CreateYawPitchRollMatrix(Matrix4x4* mat, float yaw, float pitch, float roll)
		{
			Matrix4x4 tmp;
			mat->Identity();

			CreateYawMatrix(&tmp, yaw);
			(*mat) *= tmp;
			CreatePitchMatrix(&tmp, pitch);
			(*mat) *= tmp;
			CreateRollMatrix(&tmp, roll);
			(*mat) *= tmp;
		}
		...
		// Yé≤âÒì]
		//------------------------------------------------------------------------------
		void Matrix4x4::RotationYaw(Matrix4x4* out, const Matrix4x4& in, float rot)
		{
			Matrix4x4 tmp;
			CreateYawMatrix(&tmp, rot);
			*out = in * tmp;
		}
		...
		// Xé≤âÒì]
		//------------------------------------------------------------------------------
		void Matrix4x4::RotationPitch(Matrix4x4* out, const Matrix4x4& in, float rot)
		{
			Matrix4x4 tmp;
			CreatePitchMatrix(&tmp, rot);
			*out = in * tmp;
		}
		...
		// Zé≤âÒì]
		//------------------------------------------------------------------------------
		void Matrix4x4::RotationRoll(Matrix4x4* out, const Matrix4x4& in, float rot)
		{
			Matrix4x4 tmp;
			CreateRollMatrix(&tmp, rot);
			*out = in * tmp;
		}
		...
		// ëSÇƒÇÃé≤âÒì]
		//------------------------------------------------------------------------------
		void Matrix4x4::RotationYawPitchRoll(Matrix4x4* out, const Matrix4x4& in, float yaw, float pitch, float roll)
		{
			Matrix4x4 tmp;
			CreateYawPitchRollMatrix(&tmp, yaw, pitch, roll);
			*out = in * tmp;
		}
		...
		// Yé≤âÒì]
		//------------------------------------------------------------------------------
		void Matrix4x4::RotationYaw(float rot)
		{
			RotationYaw(this, *this, rot);
		}

		//------------------------------------------------------------------------------
		// Xé≤âÒì]
		//------------------------------------------------------------------------------
		void Matrix4x4::RotationPitch(float rot)
		{
			RotationPitch(this, *this, rot);
		}

		//------------------------------------------------------------------------------
		// Zé≤âÒì]
		//------------------------------------------------------------------------------
		void Matrix4x4::RotationRoll(float rot)
		{
			RotationRoll(this, *this, rot);
		}

		//------------------------------------------------------------------------------
		// ëSÇƒÇÃé≤âÒì]
		//------------------------------------------------------------------------------
		void Matrix4x4::RotationYawPitchRoll(float yaw, float pitch, float roll)
		{
			RotationYawPitchRoll(this, *this, yaw, pitch, roll);
		}

		//------------------------------------------------------------------------------
		// ÉXÉPÅ[Éã
		//------------------------------------------------------------------------------
		void Matrix4x4::Scaling(Matrix4x4* out, const Matrix4x4& in, float x, float y, float z)
		{
			out->a._11 = x * in.a._11;
			out->a._12 = x * in.a._12;
			out->a._13 = x * in.a._13;
			out->a._21 = y * in.a._21;
			out->a._22 = y * in.a._22;
			out->a._23 = y * in.a._23;
			out->a._31 = z * in.a._31;
			out->a._32 = z * in.a._32;
			out->a._33 = z * in.a._33;
		}
		...
		{
			Scaling(this, *this, x, y, z);
		}

		//------------------------------------------------------------------------------
		// ï¿çsà⁄ìÆ
		//------------------------------------------------------------------------------
		void Matrix4x4::Translation(Matrix4x4 *out, const Matrix4x4& in, float x, float y, float z)
		{
			out->a._41 = in.a._41 + x;
			out->a._42 = in.a._42 + y;
			out->a._43 = in.a._43 + z;
		}
